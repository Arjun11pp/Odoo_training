...
class MultisafepayController(http.Controller):
    ...
    def multisafepay_return_from_checkout(self, **data):
        """Process the payment data sent by Multisafepay after redirection from checkout.

        The route is flagged with `save_session=False` to prevent Odoo from assigning a new session to the user if they are redirected to
        this route with a POST request. Indeed, as the session cookie is created without a `SameSite` attribute, some browsers that don't
    ...
    def multisafepay_webhook(self, **data):
        """Process the payment data sent by Multisafepay to the webhook.
